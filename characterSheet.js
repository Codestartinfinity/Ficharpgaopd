...
$('.ocultismBar').css('width', `${calculateBar(data.ocultism.current, data.ocultism.max)}%`)
$('#ocultismCount').text(`${data.ocultism.current}/${data.ocultism.max}`)
$('#ocultismCurrent').val(data.ocultism.current)
$('#ocultismMax').val(data.ocultism.max)

const diceModal = $('#diceAttributes')
const lifeModal = $('#lifeModal')
const sanityModal = $('#sanityModal')
const ocultismModal = $('#ocultismModal')
...
$(window).click(function (event) {
  ...
  } else if (event.target.id == 'ocultismModal') {
    ocultismModal.css('display', 'none')
...
$('.ocultismBar').click(function () {
  console.log(this)
  ocultismModal.css('display', 'block')
})
...
$('#changeOcultism').submit(function (event) {
  let current = Number($('#ocultismCurrent').val())
  const max = Number($('#ocultismMax').val())

  if (current > max) {
    alert('Os pontos atuais de ocultismo não podem ser maior que o maximo!')
    current = max
  }

  data.ocultism.current = current
  data.ocultism.max = max
  $('.ocultismBar').css('width', `${calculateBar(current, max)}%`)
  $('#ocultismCount').text(`${current}/${max}`)

  closeModal('#ocultismModal')
  event.preventDefault()
})
